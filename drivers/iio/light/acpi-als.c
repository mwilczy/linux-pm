...
static int acpi_als_add(struct acpi_device *device)
{
	...
	ret = acpi_device_install_notify_handler(device, ACPI_DEVICE_NOTIFY, acpi_als_notify);
	if (ret)
		return ret;
...
{
	acpi_device_remove_notify_handler(device, ACPI_DEVICE_NOTIFY, acpi_als_notify);
}
