...
static int chromeos_tbmc_add(struct acpi_device *adev)
{
	...
	return acpi_device_install_notify_handler(device, ACPI_DEVICE_NOTIFY, chromeos_tbmc_notify);
}

static void chromeos_tbmc_remove(struct acpi_device *device)
{
	acpi_device_remove_notify_handler(device, ACPI_DEVICE_NOTIFY, chromeos_tbmc_notify);
}
