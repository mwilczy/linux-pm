...
static int acer_wireless_add(struct acpi_device *adev)
{
	...
	return acpi_device_install_notify_handler(device, ACPI_DEVICE_NOTIFY, acer_wireless_notify);
}

static void acer_wireless_remove(struct acpi_device *adev)
{
	acpi_device_remove_notify_handler(device, ACPI_DEVICE_NOTIFY, acer_wireless_notify);
}
