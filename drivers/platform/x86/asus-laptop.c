...
static int asus_acpi_add(struct acpi_device *device)
{
	...
	result =  acpi_device_install_notify_handler(device, ACPI_ALL_NOTIFY, asus_acpi_notify);
	if (result)
		goto fail_pega_rfkill;
...
static void asus_acpi_remove(struct acpi_device *device)
{
	...
	kfree(asus->name);
	kfree(asus);
	acpi_device_remove_notify_handler(device, ACPI_ALL_NOTIFY, asus_acpi_notify);
}
