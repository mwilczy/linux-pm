...
static int rbtn_add(struct acpi_device *device)
{
	...
	return acpi_device_install_notify_handler(device, ACPI_DEVICE_NOTIFY, rbtn_notify);
...
static void rbtn_remove(struct acpi_device *device)
{
	...
	acpi_device_remove_notify_handler(device, ACPI_DEVICE_NOTIFY, rbtn_notify);
}
